...
public:
  ...
  void OnMouseMove(float x, float y, UILayoutState *uiLayoutState,
...
public:
  ...
  void AddChildMenuItem(std::string label, float x, void (*fn)()) {
    float y = hackforge::toolbar_height +
              (static_cast<float>(m_childMenuItems.size()) *
               hackforge::toolbar_line_height);
  ...
  void OnMouseMove(float x, float y, UILayoutState *mouseOver,
                   bool *continueProcessing) {
    if (IsInBounds(x, y)) {
  ...
  void Render(SDL_Renderer *renderer, UILayoutState const &mouseOver,
              SDL_Color uiColor) {
    ...
    rect.h = hackforge::toolbar_line_height *
             static_cast<float>(m_childMenuItems.size());
  ...
  size_t GetNumChildMenuItems() const {
    return m_childMenuItems.size();
  }

  void SetChildMenuItemCheckedState(size_t index, bool checkedState) {
    m_childMenuItems[index].SetCheckedState(checkedState);
  }
...
public:
  ...
  void OnMouseClick(bool *continueProcessing) {
    TopLevelMenuItem *topLevelSelected =
        dynamic_cast<TopLevelMenuItem *>(uiLayoutState.WhichItemMousedOver);
    if (topLevelSelected) {
    ...
    ChildMenuItem *childItemSelected =
        dynamic_cast<ChildMenuItem *>(uiLayoutState.WhichItemMousedOver);
    ...
    if (uiLayoutState.WhichItemExpanded != nullptr) {
      // If the user clicked back onto the canvas, then automatically collapse
      // the toolbar.
      uiLayoutState.WhichItemExpanded = nullptr;
      *continueProcessing = false;
    }
