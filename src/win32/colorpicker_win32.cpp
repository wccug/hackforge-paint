...
SDL_Color OpenNativeColorPicker(SDL_Window *window, SDL_Color currentColor) {
  ...
  if (ChooseColor(&cc) == TRUE) {
    return SDL_Color{GetRValue(cc.rgbResult), GetGValue(cc.rgbResult),
                     GetBValue(cc.rgbResult), 255};
  }
  return currentColor;
}
...
SDL_Color OpenNativeColorPicker(SDL_Window *window, SDL_Color currentColor) {
  ...
  if (dbus_error_is_set(&err)) {
    std::cerr << "[Portal] D-Bus connection failed: " << err.message
              << std::endl;
    dbus_error_free(&err);
    return currentColor;
  }
  ...
  DBusMessage *msg = dbus_message_new_method_call(
      ...
      "org.freedesktop.portal.Screenshot", "SelectColor");
  ...
  if (dbus_error_is_set(&err)) {
    std::cerr << "[Portal] Interaction canceled or failed: " << err.message
              << std::endl;
    dbus_error_free(&err);
    return currentColor;
  }
  ...
  if (dbus_message_iter_init(reply, &reply_iter)) {
    ...
    while (dbus_message_iter_get_arg_type(&results_dict) ==
